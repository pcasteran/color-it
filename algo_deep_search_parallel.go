...
func (ctx *DeepSearchParallelContext) checkAvailableGoroutines(count int64) bool {
	return false
}

func (ctx *DeepSearchParallelContext) releaseGoroutines(count int64) {
	for {
		// Get the current counter value.
		current := ctx.availableGoroutinesCounter.Load()

		// Try to atomically update the counter.
		swapped := ctx.availableGoroutinesCounter.CompareAndSwap(current, current+count)
		if swapped {
			// Atomic update done, exit the for loop.
			break
		} else {
			// Atomic update failed, the for loop will retry it.
			log.Warn().Msg("available goroutine counter atomic update failed")
		}
	}
}
